...
export type PollResult =
  | { success: true;  status: 'paid';    payment: any }
  | { success: false; status: 'failed';  message: string }
  | { success: false; status: 'pending'; message: string };
...
export async function verifyAndPollPaymentStatus(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const data = await ApiService.get<any>(`/api/payments/${encodeURIComponent(orderId)}/status`);
      ...
      if (status === 'failed') {
        return { success: false, status: 'failed', message: paymentObj?.errorMessage || 'Payment declined' };
      }
    } catch (err: any) {
      console.warn(`Payment poll attempt ${attempt} failed:`, err?.message || err);
    }
