...
const categories = ["All", "Knitwear", "Outerwear", "Blazers", "Shirts", "Footwear", "Denim"];

export default function MensPage() {
  const [selectedCategory, setSelectedCategory] = useState<string>("All");

  const { data, isLoading } = useGetProducts({ categoryName: "men", label: "men", limit: 20 });
  const allProducts = data?.pages.flatMap((page) => page) || [];

  const classifiedProducts = allProducts.map((p) => ({
    ...p,
    category: classifySubcategory(p.name, p.category || "Shirts"),
  }));
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <p className="mt-3 text-xs text-muted-foreground max-w-md leading-relaxed">
            A carefully curated edit of fine tailoring, Scotch knitwear, and Italian leather accessories designed for the modern gentleman.
          </p>
        </div>
